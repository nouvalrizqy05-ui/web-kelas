...
const MemberCarousel = () => {
    ...
    const settings = {
        ...
        responsive: [
            ...
            {
                breakpoint: 640,
                settings: { slidesToShow: 1, arrows: false, centerMode: true, centerPadding: "20px" }
            }
        ]
    }
            ...
            <Slider {...settings}>
                {members.length > 0 ? members.map((member) => (
                    <div key={member.id} className="px-3 py-5">
                        {/* Card Container Utama dengan Efek Glow Ungu Muda */}
                        <div className="relative overflow-hidden bg-[#dfdfdf10] backdrop-blur-lg border border-purple-400/50 rounded-2xl flex flex-col items-center text-center transition-all duration-300 hover:scale-[1.02] group h-full pb-8 shadow-[0_0_15px_rgba(192,132,252,0.4)] hover:shadow-[0_0_25px_rgba(192,132,252,0.6)]">
                            ...
                            {/* Background Ungu (3/4 di belakang foto) */}
                            ...
                            <div className="px-6 z-10">
                                <h3 className="text-white font-bold text-xl mb-1">{member.nama}</h3>
                                <p className="text-white/60 text-sm mb-4">{member.jabatan || "Anggota"}</p>
                                ...
                                <div className="flex justify-center gap-5 mt-4">
                                    {member.instagram_url && (
                                        <a 
                                            ...
                                            className="p-2 bg-white/5 rounded-full text-white/60 hover:text-purple-400 hover:bg-white/10 transition-all"
                                        >
